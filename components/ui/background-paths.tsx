...
/**
 * Animated flowing SVG paths that fill their positioned parent. Purely
 * decorative (`aria-hidden`, `pointer-events-none`), so it never blocks
 * interaction with content layered above it.
 *
 * Theme-aware: paths inherit `currentColor` — dark slate in light mode, white
 * in dark mode — via the `text-slate-950 dark:text-white` utility on the SVG.
 */
...
/**
 * Full-screen hero-style block that layers two mirrored `FloatingPaths` fields
 * behind an animated headline and CTA. Kept as a standalone, reusable export;
 * the portfolio Hero consumes `FloatingPaths` directly.
 */
